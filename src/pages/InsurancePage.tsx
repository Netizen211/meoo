...
export default function InsurancePage() {
  ...
  const totalCost = useMemo(() => filteredData.reduce((s: number, o: any) => s + safeFloat(findField(o, '服务费用（元）', '服务费用(元)', '服务费用', '保费（元）', '保费(元)', '保费')), 0), [filteredData]);
  const compareCost = useMemo(() => compareData.reduce((s: number, o: any) => s + safeFloat(findField(o, '服务费用（元）', '服务费用(元)', '服务费用', '保费（元）', '保费(元)', '保费')), 0), [compareData]);
  ...
  const trend = useMemo(() => {
    ...
    filteredData.forEach((o: any) => {
      ...
      if (d) byDate[d] = (byDate[d] || 0) + safeFloat(findField(o, '服务费用（元）', '服务费用(元)', '服务费用', '保费（元）', '保费(元)', '保费'));
    });
