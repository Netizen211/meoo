...
export default function SettingsPage() {
  ...
  const storageInfo = useMemo(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('dianfx_')) {
        const value = localStorage.getItem(key);
        totalSize += new Blob([value || '']).size;
        dataCount++;
      }
    }
    return { totalSize, dataCount };
  }, [clearedIds]); // 依赖 clearedIds 以在清除后刷新

  const orderCount = currentDisplayData?.orders?.length || 0;
  const promoCount = (currentDisplayData?.promotionProducts?.length || 0) + (currentDisplayData?.promotionSummary?.length || 0);
  const uploadCount = uploadRecords?.length || 0;

  const clearOptions: ClearOption[] = [
    {
      id: 'orders',
      label: '订单数据',
      description: `清除所有店铺的订单记录（${orderCount}条）`,
      icon: FileText,
      color: '#1890ff',
      bgColor: '#e6f7ff',
    ...
    },
    {
      id: 'promotion',
      label: '推广数据',
      description: `清除商品推广、明星店铺、直播推广数据（${promoCount}条）`,
      icon: TrendingUp,
      color: '#722ed1',
      bgColor: '#f9f0ff',
      action: clearPromotionData,
      warning: '清除后推广ROI、推广成本等指标将无法计算',
    },
    {
      id: 'cost',
      label: '成本配置',
      description: '清除商品成本、税费配置、包装费、运费等',
      icon: DollarSign,
      color: 'var(--pdd-warning)',
      bgColor: '#fffbe6',
      action: clearCostData,
      warning: '清除后利润计算将使用默认值，需重新配置',
    },
    {
      id: 'uploads',
      label: '上传记录',
      description: `清除文件上传历史记录（${uploadCount}条）`,
      icon: Upload,
      color: 'var(--pdd-success)',
      bgColor: '#f6ffed',
      action: clearUploadRecords,
  ...
  const handleClear = (option: ClearOption) => {
    option.action();
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
          ...
          {clearOptions.map((option, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`pdd-card p-4 border transition-all ${isCleared ? 'border-pdd-success bg-pdd-success/10' : 'border-pdd-border hover:border-pdd-primary/30'}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--pdd-text-secondary)] mb-2">{option.description}</p>
                    {option.warning && (
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => setConfirmModal(option.id)}
                      disabled={isCleared}
      ...
      <motion.div
        ...
        className="pdd-card p-5 border-2 border-pdd-danger/50"
      >
        <div className="flex items-center gap-3 mb-4">
