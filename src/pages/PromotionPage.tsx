...
export default function PromotionPage() {
  ...
  const hasPromo = (currentDisplayData?.promotionSummary?.length ?? 0) > 0;
  ...
  const comparePromoSummary = useMemo(() => {
    if (!compareEnabled || !filteredPromoSummary.length) return [];
    const dates = filteredPromoSummary.map((r: any) => safeField(r, ...PROMO_FIELDS.date).replace(/\//g, '-')).sort();
  ...
  const calcKpi = (records: any[], channel: 'promo' | 'star' | 'live') => {
    ...
    records.forEach((r: any) => {
      if (channel === 'promo') {
        ...
        cost += safeFieldVal(r, ['花费(元)']);
        gmv += safeFieldVal(r, ['交易额(元)']);
        orders += safeFieldInt(r, PROMO_FIELDS.orders);
        impressions += safeFieldInt(r, PROMO_FIELDS.impressions);
        clicks += safeFieldInt(r, PROMO_FIELDS.clicks);
  ...
  const totalKpi = useMemo(() => {
    ...
    const p = hasPromo ? calcKpi(filteredPromoSummary, 'promo') : { cost: 0, orders: 0, gmv: 0, impressions: 0, clicks: 0, ctr: 0, cvr: 0, cpc: 0, cpa: 0, inquiry: 0, favorite: 0, follow: 0 };
  ...
  }, [filteredPromoSummary, filteredStarSummary, filteredLiveSummary, filteredOrders, hasPromo, hasStar, hasLive, compareEnabled, comparePromoSummary]);

  const productKpi = useMemo(() => hasPromo ? calcKpi(filteredPromoSummary, 'promo') : null, [filteredPromoSummary, hasPromo]);
  ...
  const topProducts = useMemo(() => {
    ...
    return filteredPromoProducts
      .map((r: any) => {
        ...
        // 盈利分析
        const unitCost = productCosts[pid] || 0;
        const estimatedCost = unitCost > 0 ? unitCost * orders : gmv * (defaultRatio / 100);
  ...
  const profitAnalysis = useMemo(() => {
    ...
    const insuranceCost = filteredInsurance.reduce((s: number, r: any) => s + safeFieldVal(r, ['保费(元)', '运费险成本']), 0);
    let rawCost = 0;
    filteredOrders.forEach((o: any) => {
      ...
      const unitCost = productCosts[pid] || 0;
      rawCost += unitCost > 0 ? unitCost * qty : productTotal * (defaultRatio / 100);
    });
