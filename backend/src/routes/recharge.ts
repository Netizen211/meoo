...
// PUT /api/recharge/review/batch — 批量审核
router.put('/review/batch', requireAuth, requireRole('admin', 'test'), async (req: Request, res: Response) => {
  try {
    const { ids, action, note } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      res.status(400).json({ success: false, error: '请选择要处理的申请' });
      return;
    }
    if (!['approve', 'reject'].includes(action)) {
      res.status(400).json({ success: false, error: '无效的操作' });
    ...
    await db('admin_logs').insert({
      admin_id: req.user!.userId,
      action: action === 'approve' ? 'batch_approve_recharge' : 'batch_reject_recharge',
      target_type: 'recharge',
      target_id: ids.join(','),
      details: `批量${action === 'approve' ? '通过' : '拒绝'} ${processed}/${ids.length} 条充值申请`,
      ip_address: req.ip,
    });

    res.json({
      success: true,
      message: `已${action === 'approve' ? '通过' : '拒绝'} ${processed} 条申请`,
      processed,
    });
  } catch (err: any) {
    console.error('[recharge] batch review error:', err);
