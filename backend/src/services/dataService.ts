...
export async function loadStoreData(storeId: string): Promise<StoreDataItem | null> {
  ...
  for (const row of rows) {
    try {
      ...
      data[row.category] = Array.isArray(parsed) ? parsed.map(normalizeRecordKeys) : [];
    } catch {
      data[row.category] = [];
    }
  }
...
export async function saveUploadRecords(storeId: string, userId: string, records: UploadRecord[]): Promise<void> {
  // 标准化 datetime 格式：MySQL strict mode 不接受 ISO 格式
  const normDatetime = (dt: string) => dt ? dt.replace('T', ' ').replace('Z', '').slice(0, 19) : dt;
  for (const record of records) {
    await db('upload_records')
      .insert({
        ...
        uploaded_at: normDatetime(record.uploadedAt),
      })
      .onConflict('id')
      .merge({
        store_name: record.storeName,
...
export async function getStorageStats(): Promise<{
  ...
  return {
    totalUsers: Number((userCount as any)?.count) || 0,
    totalStores: Number((storeCount as any)?.count) || 0,
    totalRecords: Number(dataRows?.total_rows) || 0,
    storageBytes: Number(dataRows?.total_bytes) || 0,
  };
}
