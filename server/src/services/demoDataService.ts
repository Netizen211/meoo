...
function generateOrders(storeId: string): any[] {
  ...
  for (let i = 0; i < 400; i++) {
    ...
    orders.push({
      ...
      '街道/镇': city+'市'+district+pick(['中山路','人民路','解放路','建设路'])+rnd(1,300)+'号',
      '收货人': '用户'+rnd(100,999), '收货人手机': '1'+rnd(3,9)+String(rnd(100000000,999999999)).slice(0,9),
      '买家留言': Math.random()<0.15?pick(['请尽快发货','麻烦包装好一点']):'',
      '商家备注': Math.random()<0.1?pick(['老客户','加急发货']):'',
...
export async function createDemoDataForUser(userId: string): Promise<string | null> {
  try {
    ...
    await saveCategory('productCosts', [{ costs: costConfig }]);

    // 费用配置
    await db('store_configs').insert({
      store_id: storeId,
      config_key: `dianfx_cost_configs_${storeId}`,
      payload_json: JSON.stringify({
        packagingFeePerOrder: 1.5, shippingFeePerOrder: 3.0,
        platformCommissionRate: 0.6, insuranceFeePerOrder: 1.0,
        defaultCostRatio: 30, laborFeePerOrder: 2.0,
      }),
    }).onConflict(['store_id', 'config_key'] as any).merge({
      payload_json: JSON.stringify({ packagingFeePerOrder:1.5, shippingFeePerOrder:3.0, platformCommissionRate:0.6, insuranceFeePerOrder:1.0, defaultCostRatio:30, laborFeePerOrder:2.0 }),
      updated_at: db.fn.now(),
    }).catch(() => {});
