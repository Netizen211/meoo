...
export function computeAllKpis(input: ComputeKpisInput): UnifiedKpis {
  ...
  const { shippingFeePerOrder, returnShippingFeePerOrder } = config;
  ...
  const platformFee = orders.reduce((s, o) =>
  ...
  const postage = orders.reduce((s, o) => s + safeFloat(findField(o, '邮费(元)', '邮费', '快递费(元)', '快递费')), 0);
  ...
  const rfAmount = refundedAS.reduce((s, r) =>
    s + safeFloat(findField(r, '退款金额(元)', '退款金额', '买家退款金额')), 0);
  const rfCnt = new Set(refundedAS.map(r =>
    String(r['订单编号'] || r['订单号'] || '').trim()
  ).filter(Boolean)).size;
  const rfRate = cnt > 0 ? (rfCnt / cnt) * 100 : 0;

  // ★ 退款订单的商家实收合计（后端口径，用于利润计算）
  // 通过售后表找到退款成功的订单号，再取这些订单的商家实收金额(元)
  // 修复BUG：订单表没有"退款金额(元)"列，改用售后表匹配订单后取商家实收
  const refundedOrderNos = new Set(refundedAS.map(r =>
    String(r['订单编号'] || r['订单号'] || '').trim()).filter(Boolean));
  const refundedOrdersForProfit = orders.filter(o =>
    refundedOrderNos.has(String(findField(o, '订单号') || '').trim()));
  const totalRefundForProfit = refundedOrdersForProfit.reduce((s, o) =>
    s + safeFloat(findField(o, '商家实收金额(元)', '商家实收金额', '商家实收', '实收金额')), 0);
  ...
  // ── 同意退款时间维度 ──
  const { approvalDateStart, approvalDateEnd } = input;
  const approvalFilteredAS = refundedAS.filter(r => {
  ...
  // ── 运费险 ──
  const insuranceFee = insuranceRecords.reduce((s, r) =>
    s + safeFloat(findField(r, '服务费用（元）', '服务费用(元)', '保费', '保费(元)', 'insuaceFee')), 0);
  ...
  // 利润 = 商家实收 - 退款(退款订单商家实收合计) - 推广费 - 运费险 - 罚款 - 邮费 - 百亿补贴
  // 与后端口径：profit = totalRevenue - rfp - totalPromoCost - insFee - penalties - totalPostage - subsidyFee
  // 修复BUG：使用售后表匹配的退款订单商家实收，而非订单表不存在的"退款金额(元)"
  const profit = merchantReceived - totalRefundForProfit - promoCost - insuranceFee - penaltyAmount - postage - subsidyFee;
