...
export async function importSampleData(): Promise<{ storeId: string; storeName: string; results: { file: string; type: string; rowCount: number }[] }> {
  ...
  const configs: Record<string, any> = {
    [`dianfx_product_costs_${storeId}`]: demoCosts,
    [`dianfx_packaging_fee_${storeId}`]: 1.5,
    [`dianfx_shipping_fee_${storeId}`]: 3.0,
    [`dianfx_platform_commission_${storeId}`]: 0.6,
    [`dianfx_insurance_fee_${storeId}`]: 1.0,
    [`dianfx_labor_fee_${storeId}`]: 2.0,
    [`dianfx_default_cost_ratio_${storeId}`]: 30,
    [`dianfx_promotion_fee_${storeId}`]: 0,
  };

  // 上传记录
  const uploadRecords = results.map(r => ({
  ...
  // 同步到服务器
  await apiClient.post('/stores', { name: storeName }).catch(() => {});
  await apiClient.post('/data/sync', {
    storeId, storeName,
    data: storeDataForSave,
    configs,
    uploadRecords,
...
export async function hasSampleData(): Promise<boolean> {
  try {
    const res = await apiClient.get<{ id: string; name: string }[]>('/stores');
    if (res.success && res.data) {
      return res.data.some(s => s.name && s.name.includes('演示'));
    }
  } catch {}
  return false;
}
...
export async function clearSampleData(): Promise<void> {
  try {
    const res = await apiClient.get<{ id: string; name: string }[]>('/stores');
    if (res.success && res.data) {
      const demoStore = res.data.find((s: any) => s.name && s.name.includes('演示'));
      if (demoStore) {
        // ★ 删除店铺数据 + 店铺记录（云端同步删除）
        await apiClient.delete(`/data/store/${encodeURIComponent(demoStore.id)}`).catch(() => {});
        await apiClient.delete(`/stores/${encodeURIComponent(demoStore.id)}`).catch(() => {});
      }
    }
  } catch {}
}
