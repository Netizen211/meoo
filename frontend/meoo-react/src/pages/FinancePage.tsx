...
export default function FinancePage() {
  ...
  const comparisonData = useMemo(() => {
    ...
    orders.forEach((o: any) => {
      ...
      const userPaid = parseFloat(String(findField(o, '用户实付金额(元)', '用户实付', '实付金额') || '0').replace(/[^\d.\-]/g, '')) || 0;
      const productName = String(findField(o, '商品', '商品名称') || '').slice(0, 30);
      const actual = orderFinancialActuals[orderNo];

      // 公式估算（平台费基于用户实付计算，非商家实收）
      const formulaPlatformFee = userPaid * (platformCommissionRate / 100);
  ...
  const calibration = useMemo(() => {
    return getSuggestedCommissionRate(
      ...
      // 使用用户实付作为费率计算基数（平台费基于实付计算，非商家实收）
      (o: any) => parseFloat(String(findField(o, '用户实付金额(元)', '用户实付', '实付金额') || '0').replace(/[^\d.\-]/g, '')) || 0,
    );
  }, [orderFinancialActuals, orders]);
                ...
                <>
                  ...
                  <div className="pdd-card p-5">
                    ...
                    <div className="space-y-2 text-xs text-pdd-text-secondary">
                      <p>· 建议费率基于货款明细中 <b>实际技术服务费 ÷ 用户实付金额</b> 计算得出</p>
