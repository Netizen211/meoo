...
const NAV_ITEMS = [
  { path: '/stores', label: '店铺管理', icon: Store, category: 'overview' },
...
export default function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  const quickActions = [
    { icon: Download, label: '导出', onClick: () => { try { const page = NAV_ITEMS.find(n => n.path === location.pathname); if (page) exportCSV(['指标', '值'], [['页面', page.label]], `店分析_${page.label}`); } catch(e) { console.error('导出失败', e); } } },
