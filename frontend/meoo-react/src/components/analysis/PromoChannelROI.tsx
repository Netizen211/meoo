        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <PieChart>
              ...
              <Tooltip formatter={(v: number) => '¥' + (isNaN(v) ? 0 : v).toFixed(0)} />
            </PieChart>
          </ResponsiveContainer>
        </div>
        {/* 成交占比 */}
        ...
        <tbody className="divide-y divide-pdd-border/30">
          {channels.map(c => (
            <tr key={c.channel}>
              ...
              <td className="py-1 text-right font-mono">¥{(isNaN(c.cost) ? 0 : c.cost).toFixed(0)}</td>
              <td className="py-1 text-right font-mono">¥{(isNaN(c.gmv) ? 0 : c.gmv).toFixed(0)}</td>
              <td className={`py-1 text-right font-mono font-bold ${c.roi >= 2 ? 'text-green-600' : c.roi >= 1 ? 'text-yellow-600' : 'text-red-500'}`}>{(isNaN(c.roi) ? 0 : c.roi).toFixed(2)}</td>
              <td className="py-1 text-right font-mono">{(isNaN(c.ctr) ? 0 : c.ctr).toFixed(2)}%</td>
            </tr>
          ))}
        </tbody>
      </table>
