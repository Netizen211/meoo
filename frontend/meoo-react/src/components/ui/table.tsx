...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn("[&_tr]:border-b border-pdd-border", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <tr ref={ref} className={cn(
    "border-b border-pdd-border transition-colors hover:bg-pdd-gray-100 data-[state=selected]:bg-pdd-gray-100",
    className
  )} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <th ref={ref} className={cn(
    "h-12 px-4 text-left align-middle font-medium text-pdd-text-secondary text-xs uppercase tracking-wider [&:has([role=checkbox])]:pr-0",
    className
  )} {...props} />
));
